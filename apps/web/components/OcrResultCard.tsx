...
export function OcrResultCard({ item, defaultOpen = false }: Props) {
  ...
  return (
    <details
      className={`result-card ${hasError ? 'result-card--error' : ''}`}
      open={defaultOpen}
    >
      <summary className='result-card__summary'>
        <span className='result-card__name'>{item.fileName}</span>
        <span
          className={`result-card__badge ${hasError ? 'result-card__badge--error' : ''}`}
        >
                ...
                <li key={i}>
                  ...
                  {line.box != null && (
                    <span className='result-card__meta'>
                      {line.box.width}×{line.box.height} at ({line.box.x},{' '}
                      {line.box.y})
                    </span>
                  )}
                </li>
